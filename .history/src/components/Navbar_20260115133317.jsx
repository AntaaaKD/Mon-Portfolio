    ...
    <>
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 px-6 py-4 ${
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="text-2xl font-black text-[#7d1e68]">AD</div>
          <div className="hidden md:flex gap-8">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="text-sm font-medium hover:text-[#7d1e68] transition-colors uppercase tracking-widest">
                {link.name}
              </a>
            ))}
          </div>
      ...
      <div className={`fixed top-6 right-6 z-[60] transition-all duration-500 ${
        ...
        <button 
          ...
          className="w-14 h-14 bg-[#2b0a2f] text-[#0a192f] rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(34,211,238,0.5)] hover:bg-white transition-all active:scale-90"
        >
        ...
        <div className="h-full flex flex-col items-center justify-center gap-8">
          {navLinks.map((link) => (
            <a 
              ...
              className="text-3xl font-bold text-white hover:text-[#7d1e68] transition-colors uppercase tracking-tighter"
            >
              {link.name}
            </a>
          ))}
