...
return (
    <section id="achievements" className="relative w-full min-h-screen bg-transparent text-white">
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="relative">
          {/* LIGNE EN DÉGRADÉ */}
          <div className="absolute left-4 md:left-1/2 transform md:-translate-x-1/2 w-[3px] h-full bg-gradient-to-b from-cyan-400 via-cyan-600 to-transparent rounded-full opacity-50"></div>
          ...
          {achievements.map((item, index) => (
            <div key={index} className={`relative flex items-center justify-between mb-24 w-full ${index % 2 === 0 ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
              ...
              <div className="absolute left-4 md:left-1/2 transform -translate-x-1/2 w-6 h-6 rounded-full bg-[#0a192f] border-[3px] border-cyan-400 z-20 shadow-[0_0_15px_rgba(34,211,238,0.8)] flex items-center justify-center">
                 <div className="w-2 h-2 rounded-full bg-cyan-400"></div>
              </div>

              {/* CARTE */}
              <div className="w-full md:w-5/12 ml-12 md:ml-0 bg-[#112240] p-8 rounded-2xl border border-cyan-900/50 hover:border-cyan-400/50 transition-all duration-500 shadow-2xl group relative overflow-hidden">
                {item.isResearch && (
                  <div className="absolute top-0 right-0 bg-cyan-400 text-[#0a192f] text-[10px] font-bold px-3 py-1 rounded-bl-lg uppercase">
                    Research
                  </div>
                )}
                
                <span className="text-cyan-400 font-mono text-xs font-bold uppercase block mb-2">{item.date}</span>
                <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-cyan-400 transition-colors">{item.title}</h3>
                <h4 className="text-cyan-500/80 text-sm italic mb-4">{item.institution}</h4>
                ...
                {item.link && (
                  <a 
                    ...
                    className="inline-flex items-center text-cyan-400 hover:text-white transition-colors group/link"
                  >
                    <span className="text-sm font-bold uppercase tracking-widest border-b border-cyan-400 group-hover/link:border-white pb-1">
                      Read Publication
                    </span>
